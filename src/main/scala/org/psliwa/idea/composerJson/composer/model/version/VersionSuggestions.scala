...
object VersionSuggestions {
  sealed private[version] trait SimplifiedVersionConstraint {
  ...
  def suggestionsForVersions(versions: Seq[String],
                             prefix: String,
                             mostSignificantFirst: Boolean = true): Seq[String] = {
    versions
      .flatMap(suggestionsAsConstraintsForVersions(_, prefix))
      .distinct
  ...
  private[version] def parseSemantic(version: String): Option[SimplifiedVersionConstraint.Semantic] = {
    ...
    "^(v?)(\\d+)\\.(\\d+)\\.(\\d+)$".r.findFirstMatchIn(version).map { semanticMatch =>
      val pureSemanticVersion = PureSemantic(
        new SemanticVersion(toInt(semanticMatch.group(2)), toInt(semanticMatch.group(3)), toInt(semanticMatch.group(4)))
      )
  ...
  private def suggestionsAsConstraintsForVersions(version: String,
                                                  prefix: String): List[SimplifiedVersionConstraint] = {
  ...
  def suggestionsForVersion(version: String, prefix: String, mostSignificantFirst: Boolean = true): Seq[String] = {
    suggestionsAsConstraintsForVersions(version, prefix).distinct
      .sortWith((a, b) => mostSignificantFirst == isGreater(a, b))
      .map(_.presentation)
  }
  ...
  private def suggestionsForSemantic(version: Semantic,
                                     providers: List[(PureSemantic, PureSemantic) => List[SimplifiedVersionConstraint]],
                                     includeNotNormalized: Boolean = true): List[SimplifiedVersionConstraint] = {
