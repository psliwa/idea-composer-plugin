...
import org.psliwa.idea.composerJson.composer.repository.DefaultRepositoryProvider.{
...
class DefaultRepositoryProvider[Package](repositoryFactory: RepositoryFactory[Package],
  ...
  override def hasDefaultRepository(file: String): Boolean = {
    infos
      ...
      .forall(
        info =>
          info.packagist && info.urls.isEmpty || info.urls.nonEmpty && info.urls
            .forall(_.startsWith(Packagist.defaultUrl))
      )
  }
}
...
object DefaultRepositoryProvider {
  ...
  private[repository] class DefaultRepositoryFactory[Package](repositoryFromUrl: String => Repository[String],
  ...
  private def packagistRepositoryFromUrl(url: String): Option[Repository[String]] = {
    ...
    packages.toOption
      .map(
        packages =>
          Repository.callback[String](packages, memorize(30)(Packagist.loadVersions(url)(_).getOrElse(List.empty)))
      )
  }
}
