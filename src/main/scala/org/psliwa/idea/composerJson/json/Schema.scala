...
object Schema {
  ...
  private def tryDefinitions(jsonObject: JsObject): Option[Map[String,Schema]] = for {
  ...
  private def resolveDefinition(definition: JsValue, definitions: Map[String, JsValue]): JsValue = {
    def definitionByReference(ref: JsValue): Option[JsValue] = for {
    ...
    def loop(json: JsValue): JsValue = json match {
      case obj: JsObject if obj.fields.contains("$ref") => obj.fields.get("$ref").flatMap(definitionByReference).getOrElse(obj)
  ...
  private implicit def converterOps[A<:JsValue](c: Converter[A]): ConverterOps[A] = ConverterOps(c)
  private type Converter[A<:JsValue] = (A, Map[String,Schema]) => Option[Schema]

  private def jsonTypeToSchema: Converter[JsValue] = (t, defs) => t match {
  ...
  private def parseDefinitions(jsonObject: JsObject): Map[String,Schema] = {
    jsonObject.fields.flatMap{ case(name, definition) => {
      val maybeSchema = for {
  ...
  private def jsonObjectToStringSchema: Converter[JsObject] = (o, defs) => {
    for {
      ...
      pattern = Try { o.fields.get("pattern").flatMap(ensureString).map(_.r) }.toOption.flatten.map(new PatternFormat(_))
  ...
  private def jsonObjectToSimpleOr: Converter[JsObject] = (t, defs) => {
    for {
      ...
      schemaValues <- stringValues.traverse[Option,Schema](s => jsonObjectToSchema(JsObject(Map("type" -> JsString(s))), defs))
  ...
  private def jsonObjectToProperties(maybeJsonObject: JsValue, defs: Map[String,Schema]): Option[Map[String,Property]] = {
    for {
  ...
  private def jsonObjectToPatternProperties(maybeJsonObject: JsValue, defs: Map[String,Schema]): Option[Map[Regex,Property]] = {
    for {
      jsonObject <- tryJsonObject(maybeJsonObject)
  ...
  private def jsonObjectToFilePathSchema: Converter[JsObject] = (o, _) => jsonObjectToSchemaWithBooleanArg(SFilePath, "filePath", "existingFilePath")(o)

  private def jsonObjectToSchemaWithBooleanArg(f: Boolean => Schema, typeProp: String, booleanProp: String)(jsonObject: JsObject): Option[Schema] = {
  ...
  private def jsonObjectToFilePathsSchema: Converter[JsObject] = (o, _) => jsonObjectToSchemaWithBooleanArg(SFilePaths, "filePaths", "existingFilePath")(o)
