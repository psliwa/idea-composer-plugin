...
import org.psliwa.idea.composerJson.composer.repository.{TestingRepositoryProvider, CallbackRepository, DefaultRepositoryProvider, Packagist}
...
class PackagesLoader extends ApplicationComponent {
  lazy val repositoryProvider =
    if(isUnitTestMode) new TestingRepositoryProvider
    else new DefaultRepositoryProvider(packagistRepository, new BaseLookupElement(_))
