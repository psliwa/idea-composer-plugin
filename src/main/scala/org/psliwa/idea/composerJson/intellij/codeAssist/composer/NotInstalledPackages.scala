...
import org.psliwa.idea.composerJson.composer.model.{PackageName, Packages}
import org.psliwa.idea.composerJson.intellij.PsiElements._

import scala.jdk.CollectionConverters._

private object NotInstalledPackages {
  def getNotInstalledPackageProperties(element: PsiElement, installedPackages: Packages): Seq[JsonProperty] =
    for {
      ...
      if isNotInstalled(packageProperty, devPred, installedPackages)
    } yield packageProperty

  private def isNotInstalled(property: JsonProperty,
                             devPredicate: Boolean => Boolean,
                             installedPackages: Packages): Boolean = {
