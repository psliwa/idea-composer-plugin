...
class ComposedRepositoryTest extends BasePropSpec {
  ...
  property("contains packages from all repositories") {
    forAll(repositoryWithPackageNamesGen) { case(repository: Repository[String], packages: Seq[PackageName]) =>
      repository.getPackages ?= packages.map(_.presentation)
    }
  }

  property("contains versions from all repositories") {
    forAll(repositoryWithVersionsGen) { case(repository: Repository[String], pkgsVersions: Map[PackageName, Seq[String]]) =>
