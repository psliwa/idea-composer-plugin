...
object VersionGenerators {
  def semanticVersion(size: Int): Gen[SemanticConstraint] = Gen.listOfN(size, validSemanticVersionNumber)
    .map(parts => SemanticConstraint(new SemanticVersion(parts.toArray)))
  def semanticVersion: Gen[SemanticConstraint] = for {
    size <- Gen.choose(1, 3)
  ...
  def wrappedVersion(size: Int) = for {
  ...
  def wrappedVersion: Gen[Constraint] = for {
    size <- Gen.choose(1, 3)
  ...
  def logicalOperator = Gen.oneOf((LogicalOperator.AND, ","), (LogicalOperator.AND, ", "), (LogicalOperator.OR, "|"), (LogicalOperator.OR, " || "))
  def logicalVersion = for {
  ...
  def constraintOperator = Gen.oneOf(ConstraintOperator.!=, ConstraintOperator.<, ConstraintOperator.<=, ConstraintOperator.>, ConstraintOperator.>=, ConstraintOperator.^, ConstraintOperator.~)
  ...
  def constraintVersion = for {
    operator <- constraintOperator
    version <- semanticVersion
    padding <- padding
  } yield OperatorConstraint(operator, version, padding)
  def version = Gen.oneOf(semanticVersion, constraintVersion, devVersion, wildcardVersion, wrappedVersion, operatorVersion, hashVersion, hyphenRangeVersion, aliasedVersion, logicalVersion)
