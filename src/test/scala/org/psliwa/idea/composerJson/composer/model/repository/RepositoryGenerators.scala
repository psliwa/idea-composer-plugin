...
object RepositoryGenerators {
  ...
  implicit private val seqStringSemigrup: Semigroup[Seq[String]] = seqSemigroup[String]

  private def pkgGen: Gen[PackageName] = Gen.listOfN(5, Gen.alphaLowerChar).map(_.mkString("")).map(PackageName)
  private def versionGen: Gen[String] =
    for {
      size <- Gen.choose(1, 2)
      chars <- Gen.listOfN(size, Gen.alphaChar)
    } yield chars.mkString("")
  private def versionsGen: Gen[List[String]] =
    for {
      size <- Gen.choose(0, 50)
      versions <- Gen.listOfN(size, versionGen)
    } yield versions
  def repositoryWithPackageNamesGen: Gen[(Repository[String], Seq[PackageName])] =
    for {
      n <- Gen.choose(0, 20)
  ...
  private def pkgsVersionsGen(packageNames: Seq[PackageName]): Gen[Map[PackageName, Seq[String]]] =
    for {
      packagesAndVersions <- Gen.sequence[Seq[(PackageName, Seq[String])], (PackageName, Seq[String])](
                              packageNames.map(packageName => versionsGen.map(packageName -> _))
                            )
    } yield packagesAndVersions.toMap

  def repositoryWithVersionsGen: Gen[(Repository[String], Map[PackageName, Seq[String]])] =
    for {
